...
for BMK in ${BENCHMARKS[@]}; do
  ...
  INPUT_FILES=( $(ls -1 perf*.data) )

  echo "${SUITE_EXE} ${SUITE_DATA_TYPE}" > $OUTS

  for INPUT_FILE in "${INPUT_FILES[@]}"; do
    echo "using input file: ${INPUT_FILE}" > $OUTS
    OUTPUT_PREFIX=${INPUT_FILE%*.data}

    if [[ $SCRIPT_DRYRUN -eq 0 ]]; then
      if [[ $SCRIPT_JOBS -gt 1 ]]; then
        sem --no-notice -j ${SCRIPT_JOBS} "taskset -c ${CPU_NUM} perf script -i ${INPUT_FILE} | c++filt | gprof2dot.py -f perf -o ${OUTPUT_PREFIX}.dot && dot -Tpdf -o ${OUTPUT_PREFIX}.pdf ${OUTPUT_PREFIX}.dot"
      else
        taskset -c ${CPU_NUM} perf script -i ${INPUT_FILE} | c++filt | gprof2dot.py -f perf -o ${OUTPUT_PREFIX}.dot && dot -Tpdf -o ${OUTPUT_PREFIX}.pdf ${OUTPUT_PREFIX}.dot
      fi
    fi
