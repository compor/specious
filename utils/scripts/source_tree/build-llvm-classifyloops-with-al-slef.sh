...
PIPELINE_CONFIG_FILE="${SRC_DIR}/configs/pipelines/classifyloops_with_al_slef.txt"

[[ -z "${CLASSIFYLOOPS_DIR}" ]] && echo "error: CLASSIFYLOOPS_DIR is not set" && exit 1
[[ -z "${ANNOTATELOOPS_DIR}" ]] && echo "error: ANNOTATELOOPS_DIR is not set" && exit 1
[[ -z "${SIMPLIFYLOOPEXITSFRONT_DIR}" ]] && echo "error: SIMPLIFYLOOPEXITSFRONT_DIR is not set" && exit 1
...
CC=clang CXX=clang++ \
  ...
  -DClassifyLoops_DIR="${CLASSIFYLOOPS_DIR}" \
